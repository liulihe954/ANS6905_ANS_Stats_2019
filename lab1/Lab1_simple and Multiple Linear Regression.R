...
plot_simple_lm = ggplot(data = BodyWeightData_sub,aes(x = DMI, y = BW)) + geom_point(color='blue')
plot_simple_lm
dev.off()
# Fit a module, recall our formula (Y = ß0 + ß1 * x + ε)
BW_simple_lm <- lm(BW ~ DMI, # regression formula (R style syntax)
...
y = BodyWeightData_sub$BW # Vector of Responses
# x'x
t(x) %*% x 
# (x'x)^(-1)
solve(t(x) %*% x ) 
...
BW_categ_lm_full <- lm(BW ~ DMI + Diet + Trt, # regression formula (R style syntax)
...
BW_categ_lm_nested <- lm(BW ~ DMI + Trt , # regression formula (R style syntax)
                       data= BodyWeightData) # speficy your dataset; R will search/match the variable names
...
# Pay attention to the name of every predictor
summary(BW_categ_lm_full)
BodyWeightData_new_ref <- within(BodyWeightData,Diet <-relevel(Diet,ref = "LC"))
BW_categ_lm_new_ref <- lm(BW ~ DMI + Diet + Trt, # regression formula (R style syntax)
...
# checking the model in matrix - graphical impression
# calculating by hands
# parameter estimate (least squares estimator)
# variance - covariance matrix
# Estimation of residual variances
# t-test of the significance single predictor
