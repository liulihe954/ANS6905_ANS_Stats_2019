...
library(tidyverse);library(faraway);library(locfit)
```
...
1.when two or more explanatory variables are moderately or highly correlated impacts both estimation and inference  
2. large changes in estimate & significance of β^hat when one or more explanatory variables are added or deleted  
...
Or we use function vif() unction from other package  
```{r}
...
```
check cor coef, so high...  
Additional predictors offer no mre explanatory effect when some are already included.  
...
# One approach: 
given a criterion, fit all possible models, and then identify the best one
