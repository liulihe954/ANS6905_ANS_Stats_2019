...
Parity = c(rep("P",240) ,rep("M",560))
Dystocia = c(rep("D",35),rep("ND",205),rep("D",35),rep("ND",525))

data = data.frame(Parity = Parity, Dystocia = Dystocia) ## create data
head(data);tail(data)

CT = ftable(Dystocia ~ Parity, data)
CT ## create a 2-by-2 table
data$Parity = relevel(data$Parity, "P") # probability of Dystocia is same among different parity levels
